...
import { PencilIcon, UserAddIcon, UserRemoveIcon, RefreshIcon, SearchIcon, KeyIcon } from '@heroicons/react/solid';
...
const Configuracion = () => {
    ...
    const okInsertar = async (value) => {
        ...
        console.log(value);
        if (value)
            await postUsuarios(value);
    }
    /* Inicio Peticiones*/
    const getRoles = async () => {
        await get(`roles/get`).then((items) => {
            items !== undefined ? setRoles(items) : setRoles([]);
        });
    }
    const getUsuarios = async () => {
        await get('users/get').then((items) => {
            items !== undefined ? setUsuarios(items) : setUsuarios([]);
        });
    }
    const postUsuarios = async (values) => {
        await post('users/add', values).then((items) => {
    ...
    return (
        <Fragment>
            {openInsertar ? <Insertar openInsert={activeInsertar} roles={roles} confirm={okInsertar} /> : <></>}
