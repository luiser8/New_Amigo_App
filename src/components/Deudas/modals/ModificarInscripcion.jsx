...
const ModificarInscripcion = ({ openC, confirm, planDePago }) => {
                ...
                >
                    <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                            ...
                            <div className="sm:flex sm:items-start">
                                ...
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                                        Modificar Inscripción <span style={{color:'red'}}>{planDePago}</span>
                                    </Dialog.Title>
                                            ...
                                            <div className="px-4 py-5 bg-white sm:p-6">
                                                <div className="grid grid-cols-2 gap-6">
                                                    <div className="col-span-10 sm:col-span-3">
                                                        <label htmlFor="country" className="block text-sm font-medium text-gray-700">Tipo de Ingreso</label>
                                                            <select
                                                                id="arancel"
                                                                name="arancel"
                                                                className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                                                onChange={async (event) => changeTipoIngreso(event.target.value)}
                                                            >
                                                                <option value={0}>Seleciona Tipo de Ingreso</option>
                                                                {Object.keys(tipos).map((key, tipo) => (
                                                                    <option key={key} value={`${tipos[tipo].Id_TipoIngreso}`} >{tipos[tipo].Descripcion}</option>
                                                                ))}
                                                            </select>
                                                        </div>
                                                </div>
                                                <div className="grid grid-cols-2 gap-6">
                                                    <div className="col-span-10 sm:col-span-3">
                                                        <label htmlFor="country" className="block text-sm font-medium text-gray-700">Plan de Pago</label>
                                                            <select
                                                                id="arancel"
                                                                name="arancel"
                                                                className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                                                onChange={async (event) => changePlanDePago(event.target.value)}
                                                            >
                                                                <option value={0}>Seleciona Plan de Pago</option>
                                                                {Object.keys(planes).map((key, plan) => (
                                                                    <option key={key} value={`${planes[plan].Id_Plan}`} >{planes[plan].Descripcion}</option>
                                                                ))}
                                                            </select>
                                                        </div>
                                                </div>
                        ...
                        <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button
                                type="button"
                                disabled={id_plan !== 0 && id_tipoIngreso !== 0 ? false : true}
                                className={`w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 ${id_plan !== 0 && id_tipoIngreso !== 0 ? 'bg-green-600 hover:bg-green-700' : 'bg-green-200 hover:bg-green-200'} text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm`}
