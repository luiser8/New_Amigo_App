...
import {
  getPlanesSAIAService,
  getPlanesService,
} from "../../services/planesService";
import {
  postCuotaAllSAIAService,
  postCuotaAllService,
...
const Insertar = () => {
  ...
  const [cuota, setCuota] = useState(checkConfigN().Cuota);
  const [cuotaSAIA, setCuotaSAIA] = useState(checkConfigI().CuotaSAIA);
  ...
  const postCuota = async (ev) => {
    ...
    let data = {
      Id_Periodo:
        id_periodo === undefined || id_periodo === 0
          ? lapsos.filter((x) => x.Lapso === lapso)[0].Id_Periodo
          : id_periodo,
      Lapso: lapso,
      Monto: cuota,
    ...
    setLoading(false);
    Toast({ show: false });
    getAranceles("normal", checkLapso(), 2);
    getPlanes("normal", checkLapso());
  };
  ...
  const postCuotaSAIA = async (ev) => {
    ...
    setLoading(false);
    Toast({ show: false });
    getAranceles("saia", checkLapso(), 3);
    getPlanes("saia", checkLapso());
  };
  ...
  const changeArancelFechaSAIA = async (value) => {
    ...
    if (value !== 0 && fecha !== undefined) {
      setFechaVencimientoSAIA(
        moment(fecha.FechaVencimiento).format("DD-MM-YYYY"),
      );
    } else {
      setFechaVencimientoSAIA("");
    }
  ...
  useEffect(() => {
    getLapsos();
    getAranceles("normal", checkLapso() !== null ? checkLapso() : lapso, 2);
    getAranceles("saia", checkLapso() !== null ? checkLapso() : lapso, 3);
    getPlanes("normal", checkLapso() !== null ? checkLapso() : lapso);
    getPlanes("saia", checkLapso() !== null ? checkLapso() : lapso);
    return () => {
      setLapsos([]);
      setAranceles([]);
      setArancelesSAIA([]);
                ...
                <div className="mt-10 sm:mt-0">
                  <div className="md:grid md:grid-cols-2 md:gap-6">
                    <div className="col-span-0">
                      ...
                      >
                        <option value={0}>Selecciona cuota a insertar</option>
