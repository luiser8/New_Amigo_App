...
const PanelEdicion = ({
  deudas,
  facturas,
  activeModificacionCedula,
  activeInsertar,
  activeModificacionInsc,
}) => {
  return (
    <Fragment>
      <div className="mt-6 flex">
        <span className="hidden sm:block">
          ...
          >
            <PencilIcon
              className={`mr-0 h-8 w-8 ${Object.keys(deudas).length !== 0 || Object.keys(facturas).length !== 0 ? "text-gray-500 cursor-pointer" : "text-gray-100 cursor-no-drop"}`}
              aria-hidden="true"
            />
          </button>
        </span>
