...
const Login = () => {
  ...
  const handleLogin = async (event) => {
    setLoadingBtn(true);
    setLoadingBtnMsj("Entrando... Espere un momento.");
    ...
    if (loginHandle !== undefined || loginHandle.length !== 0) {
      if (loginHandle.length !== 0 && loginHandle[0].Bloqueado === 0) {
        login(0, {
          UsuarioId:
            loginHandle[0].UsuarioId !== undefined
              ? loginHandle[0].UsuarioId
              : "",
          Nombres:
            loginHandle[0].Nombres !== undefined ? loginHandle[0].Nombres : "",
          Apellidos:
            loginHandle[0].Apellidos !== undefined
              ? loginHandle[0].Apellidos
              : "",
          Rol: loginHandle[0].Rol !== undefined ? loginHandle[0].Rol : "",
          NombreRol:
            loginHandle[0].NombreRol !== undefined
              ? loginHandle[0].NombreRol
              : "",
        });
      } else if (loginHandle.length === 0) {
        setError(true);
        setErrorMsjTitle("Error de sesión");
        setErrorMsj(
          "Ocurrió un problema intentando iniciar la sesión. Usuario / Contraseña erronea, intenta de nuevo",
        );
      } else if (loginHandle[0].Bloqueado === 1) {
        setError(true);
        setErrorMsjTitle("Error de sesión");
        setErrorMsj(
          "Ocurrió un problema intentando iniciar la sesión. Usuario bloqueado temporalmente",
        );
      }
    } else {
      setError(true);
      setErrorMsjTitle("Error de red");
      setErrorMsj("Ocurrió un error en la comunicación");
    }
    setLoadingBtn(false);
    setLoadingBtnMsj("Entrar");
  };
  ...
  return (
    <Fragment>
      ...
      <div
        className={`${
          error !== true ? "min-h-screen" : ""
        } flex items-center justify-center ${
          error !== true ? "py-10" : "py-72"
