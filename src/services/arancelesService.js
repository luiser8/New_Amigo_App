import {
  getArancelesClient,
  getArancelesSAIAClient,
  getArancelesPorLapsoClient,
} from "../clients/arancelesClient";
...
export const getArancelesPorLapsoService = async (lapso) => {
  let aranceles = [];
  Promise.all([
    await getArancelesPorLapsoClient(lapso).then((values) => {
      if (values !== null) {
        aranceles = [...aranceles, ...(values !== undefined ? values : [])];
      }
    }),
