import {
  getReporteMenuCarrerasClient,
  getReporteMenuClient,
  getReporteFacturacionPorDepositoslient,
...
export const getReporteMenuCarrerasService = async (
  idPeriodo,
  desde,
  hasta,
) => {
  ...
  Promise.all([
    await getReporteMenuCarrerasClient(idPeriodo, desde, hasta).then(
      (values) => {
        if (values !== null) {
          reporteMenuCarreras = [
            ...reporteMenuCarreras,
